...
const Review = () => {
  ...
  // GraphQL query to get posts specifically from the 'reviews' custom post type
  // Assuming your custom post type name in GraphQL schema is 'reviews' (lowercase 'r')
  const REVIEWS_POSTS_QUERY = `
    query GetReviewsCustomPosts($first: Int!, $after: String) {
      reviews(first: $first, after: $after, where: { orderby: { field: DATE, order: DESC } }) {
        nodes {
          id
          title
          date
  ...
  const loadReviewPosts = async () => {
    if (!hasMorePosts && reviewPosts.length > 0) {
      return;
    }

    const isInitialLoad = reviewPosts.length === 0;
    ...
    const data = await fetchGraphQL(REVIEWS_POSTS_QUERY, {
      first: POSTS_PER_PAGE,
      after: isInitialLoad ? null : endCursor,
    });

    if (data && data.reviews && data.reviews.nodes) {
      setReviewPosts((prevPosts) => [...prevPosts, ...data.reviews.nodes]);
      setEndCursor(data.reviews.pageInfo.endCursor);
      setHasMorePosts(data.reviews.pageInfo.hasNextPage);
    } else {
      setReviewPosts([]);
  ...
  useEffect(() => {
    loadReviewPosts(); // Initial load when component mounts
  }, []);
  ...
  const renderPostCard = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => console.log("Navigating to review URI:", item.uri)}
    >
  ...
  const renderFooter = () => {
    if (!loadingMore && !hasMorePosts) return null; // Only show footer if loading or has more posts
    if (!loadingMore && hasMorePosts) { // Show a button if not loading but more posts available
      return (
  ...
  if (loading && reviewPosts.length === 0) {
    return (
      ...
      <FlatList
        ...
        ListHeaderComponent={() => (
          <View style={styles.pageHeaderSection}>
            ...
            <Text style={styles.pageSubtitle}>In-depth reviews of the latest devices.</Text>
          </View>
        )}
        ListFooterComponent={renderFooter}
        ListEmptyComponent={!loading && !loadingMore && reviewPosts.length === 0 && (
          <Text style={styles.noContentText}>No reviews found.</Text>
        )}
      />
    </SafeAreaView>
  );
